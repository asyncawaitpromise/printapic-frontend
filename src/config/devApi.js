...
const API_CONFIG = {
  ...
  USE_DEV_API: import.meta.env?.MODE === 'development' || !import.meta.env?.VITE_API_URL,
  ...
  SIMULATION_PROCESSING_TIME: 15000, // 15 seconds for AI processing
  SIMULATION_QUEUE_SIZE: 3,          // Simulate 3 jobs in queue
  SIMULATION_TOKEN_COST: 2,          // 2 tokens per AI processing
...
export const simulateDelay = (operation = 'default') => {
  const delay = API_CONFIG.SIMULATION_DELAYS[operation.toUpperCase()] || 1000;
...
/**
 * Create response with realistic structure
 * @param {*} data - The response data
 * @param {boolean} success - Whether the response is successful
 * @param {string} message - Response message
 * @returns {Object} - Mock API response
 */
export const createResponse = (data = null, success = true, message = '') => {
  return {
    success,
    data,
    message,
...
/**
 * Development HTTP client with realistic behavior
 */
export class DevHttpClient {
  constructor() {
  ...
  async request(method, endpoint, data = null, options = {}) {
    ...
    // Log dev request
    console.log(`🔄 Dev ${method.toUpperCase()} ${endpoint}`, data ? { data } : '');
    
    // Return response based on endpoint
    return this.getResponse(method, endpoint, data);
  }
  ...
  getResponse(method, endpoint, data) {
    // Responses based on endpoint patterns
    ...
    if (endpoint.includes('/photos/upload')) {
      return createResponse({
        ...
        uploadUrl: 'https://dev-storage.com/photo.jpg',
        status: 'uploaded'
      });
    }
    ...
    if (endpoint.includes('/ai/process')) {
      return createResponse({
        jobId: 'job_' + Date.now(),
        status: 'queued',
        estimatedTime: API_CONFIG.SIMULATION_PROCESSING_TIME,
        queuePosition: API_CONFIG.SIMULATION_QUEUE_SIZE,
        tokenCost: API_CONFIG.SIMULATION_TOKEN_COST
      });
    }
    ...
    // Default response
    return createResponse({ message: 'Development response' });
  }
}

// Create singleton instance
export const httpClient = new DevHttpClient();
